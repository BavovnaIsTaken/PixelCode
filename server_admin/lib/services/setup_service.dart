...
class SetupService extends ChangeNotifier {
  ...
  bool get isBusy => _isRunning || _isRollingBack || _isProbing;
...
''';
  ...
  // ─── Probe (non-destructive read of current env state) ───────────────────

  // Detects "already set up" on launch so the SETUP page doesn't lie about
  // pending steps when the daemon and server are already running.
  Future<void> probeExisting() async {
    if (isBusy || _isDone) return;
    ...
    try {
      ...
      final claudeR = await _shell('which claude');
      final claudeOk = claudeR.exitCode == 0;
      if (claudeOk) _claudePath = claudeR.stdout.toString().trim();
      ...
      final nodeOk = results[3].exitCode == 0;
      final npmOk  = results[4].exitCode == 0;
      _sysInfo = {
        'arch':  results[0].stdout.toString().trim(),
        'macos': results[1].stdout.toString().trim(),
        'brew':  results[2].exitCode == 0 ? results[2].stdout.toString().trim() : 'not installed',
        'node':  nodeOk ? results[3].stdout.toString().trim() : 'not installed',
        'npm':   npmOk  ? results[4].stdout.toString().trim() : 'not installed',
      };
      _envExistedBefore = File('$serverDir/.env').existsSync();

      final nodeModulesOk = Directory('$serverDir/node_modules').existsSync();
      final envOk = _envExistedBefore;

      final now = DateTime.now();
      if (claudeOk) _markStep('find_claude', StepStatus.done, now);
      if (claudeOk && nodeOk && npmOk) _markStep('scan', StepStatus.done, now);
      if (claudeOk && nodeOk && npmOk && nodeModulesOk && envOk) {
        _markStep('setup', StepStatus.done, now);
        _markStep('verify', StepStatus.done, now);
        _isDone = true;
      }
    } catch (_) {
      // Silent on probe failure — user can still click "Розпочати".
    } finally {
      _isProbing = false;
      notifyListeners();
    }
  ...
  void _markStep(String id, StepStatus status, DateTime ts) {
    for (final step in _steps) {
      if (step.id == id) {
        step.status = status;
        step.timestamp = ts;

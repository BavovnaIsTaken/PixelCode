...
export interface HealthContext {
  ...
  /** Absolute path to the Flutter project root (used for Android signing check). */
  projectCwd: string;
}
...
async function checkAndroidSigning(ctx: HealthContext): Promise<HealthItem> {
  // Flutter standard: android/key.properties wires a release keystore.
  const keyProps = join(ctx.projectCwd, "android", "key.properties");
  if (existsSync(keyProps)) {
    return { id: "androidSigning", status: "ok", fixable: false, detail: "android/key.properties знайдено (release keystore)" };
  }
  // Debug keystore is auto-generated by the Android SDK — acceptable for sideloading.
  ...
  if (existsSync(debugKeystore)) {
    return {
      ...
      status: "ok",
      fixable: false,
      detail: "Debug keystore (~/.android/debug.keystore) — сайдлоадінг OK",
    };
  }
  return {
    id: "androidSigning",
    status: "fail",
    fixable: false,
    detail: "Keystore не знайдено",
    instruction:
      "Для сайдлоадінгу APK запусти хоча б один build щоб SDK згенерував debug keystore. Для Play Store:\n\n" +
      "1. Згенеруй keystore:\n   keytool -genkey -v -keystore ~/key.jks -keyalg RSA -keysize 2048 -validity 10000 -alias key\n\n" +
      "2. Створи android/key.properties:\n   storePassword=<пароль>\n   keyPassword=<пароль>\n   keyAlias=key\n   storeFile=/Users/<user>/key.jks\n\n" +
      "3. Підключи signing config в android/app/build.gradle.",
  };
}
...
export async function runSingleCheck(id: HealthItemId, ctx: HealthContext): Promise<HealthItem | null> {
  switch (id) {
    ...
    case "androidSigning":     return checkAndroidSigning(ctx);

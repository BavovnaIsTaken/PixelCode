...
| Компонент | Статус | Notes |
|---|---|---|
| **Furniture placement/edit-mode fix + move** (pencil-toggle перестав вмикати edit mode після Stage 2 furniture-tab migration — `furnitureEditModeProvider` був перероблений у derived, але пенцил-button все ще намагався toggle-нути його; Stage 2 placement path тимчасово зник з in-build-mode гілки tap router-а; пенцил має один tooltip "Видалити кімнати" а робить delete+move обидва — twin-bug fixed одним заходом) | `[DONE]` | 2026-05-19 — три providers: `furnitureEditModeProvider` (StateProvider<bool>, independent pencil-toggle), `selectedFurnitureIdProvider` (inventory placement hand), `heldPlacedFurnitureIndexProvider` (move pickup hand). Coordination через `FurnitureEditModeCoord` static helpers — enforce mutual-exclusion at write-sites. `_handleEditModeTap` рівяв три-state machine: held → move | tap-same → cancel; selected → place; empty hand → pick-up furniture / delete room. Long-press → delete furniture. Painter рендерить held item з pulsing green outline + inner glow щоб гравець бачив що "в руках". X-button у build menu тепер також ексetzt furniture coord щоб edit-mode не лишався true після виходу. Narrow layout (mobile) отримав свій pencil. [lib/providers/shop_navigation_provider.dart](../lib/providers/shop_navigation_provider.dart), [lib/widgets/canvas/agent_canvas.dart](../lib/widgets/canvas/agent_canvas.dart) `_handleEditModeTap` + `_handleEditModeLongPress` + `_canPlaceFurnitureAt` + `_hitTestPlacedFurniture`, [lib/widgets/canvas/build_menu.dart](../lib/widgets/canvas/build_menu.dart), [lib/widgets/canvas/build_decor_section.dart](../lib/widgets/canvas/build_decor_section.dart), [lib/widgets/canvas/pixel_office_painter.dart](../lib/widgets/canvas/pixel_office_painter.dart). Тести: [test/widgets/build_decor_section_test.dart](../test/widgets/build_decor_section_test.dart) — coord invariants + Скасувати semantics. |
| **Room move** (drag/tap-and-tap relocation для placed rooms; furniture move вже [DONE] вище) | `[TODO]` | Q3 2026 — потребує `moveRoom(roomId, newCol, newRow)` provider method (зараз тільки `removeRoom`), плюс рішення про cascade-move-with-furniture vs only-empty-rooms. 90% користувацької цінності move — у furniture (rooms ставляться раз, меблі переставляються часто); тому room move лишається follow-up до D-phase office personalization, де офіс стає custom-agent-ready. |
| **Stage 4 decision gate — fixed-tier shells vs free-form** | `[RESOLVED 2026-07-14]` | 2026-05-12 OPEN → 2026-05-14 TRIGGERED після playtest-у власника → 2026-07-14 RESOLVED (expansion вирізано, fixed-tier shells реалізовано — див. Stage 4 нижче). Дві model-mismatch помилки підряд (клік на amber-buffer = "купую цю клітинку"; купівля = "ряд/стовбчик, а не тайл") підтвердили що per-row/col expansion не intuit-ується навіть автором механіки. Ріжемо expansion mechanic, фіксуємо canvas-grid per tier, залишаємо furniture+room-placement editor (Hex "IKEA, не Minecraft"). |
...
| **Persist canvas pan + zoom** | `[DONE]` | 2026-07-14 — реалізовано як окремий Riverpod-провайдер [lib/providers/office_view_provider.dart](../lib/providers/office_view_provider.dart) (`OfficeViewNotifier extends Notifier<Matrix4>`), а не in-widget load/save. Transform дзеркалиться у SharedPreferences (`office_view_transform`), відновлюється на mount ([agent_canvas.dart:128](../lib/widgets/canvas/agent_canvas.dart#L128)), зберігається по завершенню жесту (`onInteractionEnd`→`save`, ряд 403-404). Build mode заходить у identity + виходить у збережений transform (ряд 209), тому build-pan не тече у persisted value. Замість per-tier ключа — `_normalize` clamp-ить scale у viewer-range [1.0, 3.0] і скидає translation коли un-zoomed, тож corrupt/stale value не може застрандити офіс off-screen (простіше й безпечніше за per-tier-key). Тести: [test/providers/office_view_provider_test.dart](../test/providers/office_view_provider_test.dart) (roundtrip + clamp + normalize). |
| **Grid-інвентар** (RPG/Minecraft-style сітка клітинок власних предметів: меблі + wall/floor skin-паки; tap меблі → Build Mode «в руки»; tap пак → секція Стіни/Підлога; long-press drag для перестановки, «Сортувати» re-group; порядок персиститься на `GameState.inventoryOrder`; свідомо НЕ дублює косметику з Налаштувань) | `[DONE]` | 2026-07-14 — новий Hub-таб. [lib/widgets/inventory/inventory_panel.dart](../lib/widgets/inventory/inventory_panel.dart), `inventoryOrder` у [lib/models/game_economy.dart](../lib/models/game_economy.dart) + провайдер, wired у [hub_screen.dart](../lib/screens/hub/hub_screen.dart). Snap-логіка канваса винесена зі `snap_logic.dart` → [lib/widgets/canvas/build_mode_logic.dart](../lib/widgets/canvas/build_mode_logic.dart). Тести: inventory_panel + inventory_order + build_mode_logic (66 зелених). Timed-trial «пробник» ([IDEAS.md](IDEAS.md)) навмисно deferred. |
...
Реалізовано 2026-07-14 (одним заходом, 2432 тести зелені). Fixed grid per tier = старий fully-expanded max, тож legacy saves колапсуються без втрати кімнат.

| Компонент | Статус | Файли |
|---|---|---|
| **Model rip** (`OfficeExpansion`/`PendingExpansionPlan`/`computeExpansionPlan`/`effectiveCols/Rows`/`.expansions`/`nextExpansion`/`isOfficeFullyExpanded` видалено; `gridCols/Rows` → const per tier = старий max: garage 10×7, small 13×10, modern 16×12, techHub 18×15, campus 22×20, galley 22×54; `playableTiles` no-arg getter) | `[DONE]` | 2026-07-14 — [lib/models/game_economy.dart](../lib/models/game_economy.dart) |
| **Provider rip** (`canBuyOfficeExpansion`/`buyOfficeExpansion`/`_planExpansion`/`_ExpansionPurchasePlan`/`expansionStepsToBuy` видалено; `upgradeOffice`+`switchToOffice` → спільний `_withOfficeLot` wallet-check + place) | `[DONE]` | 2026-07-14 — [lib/providers/game_economy_provider.dart](../lib/providers/game_economy_provider.dart) |
| **Canvas rip** (`_drawFoundationBuffer`+`_formatCompact`, amber buffer, `_bufferDims`, `_BuildBanner`, `GhostInvalidReason.bufferOverrun`+`tierCeiling`, `GhostStatus.pendingExpand`, `_bufferTileAt`/`_updateBufferHover`, `_bufferHoverFade`, `kBuildBufferTiles`, `_showFoundationBuffer`, `_tryBuyExpansionAtTap`, painter buffer-поля — видалено; ghost або влазить у fixed lot або `outOfBounds`) | `[DONE]` | 2026-07-14 — [lib/widgets/canvas/build_mode_logic.dart](../lib/widgets/canvas/build_mode_logic.dart), [lib/widgets/canvas/agent_canvas.dart](../lib/widgets/canvas/agent_canvas.dart), [lib/widgets/canvas/pixel_office_painter.dart](../lib/widgets/canvas/pixel_office_painter.dart) |
| **Save schema v6→v7 migration** (не v5→v6 — код уже був на v6 зі Stage 1; drop `officeExpansions`; `legacyExpansionRefund` = 100% суми оплачених кроків на поточному tier зі snapshot-таблиці `_legacyExpansionCostsV6`; refund гейтований на `version<7 && field present` → одноразовий; rooms не випадають бо grid тільки росте) | `[DONE]` | 2026-07-14 — [lib/models/game_economy.dart](../lib/models/game_economy.dart) `fromJson` + migration test у [test/models/game_state_test.dart](../test/models/game_state_test.dart) |
| **Tier-upgrade dialog rework** (expansion-progress pips + buy-chip видалено; current-tier card → «Лот NxM • N клітинок») | `[DONE]` | 2026-07-14 — [lib/widgets/canvas/office_upgrade_dialog.dart](../lib/widgets/canvas/office_upgrade_dialog.dart), shop office-card CTA `expansion`→видалено ([shop_panel.dart](../lib/widgets/shop/shop_panel.dart), [shop_panel_helpers.dart](../lib/widgets/shop/shop_panel_helpers.dart), [office_upgrade_helpers.dart](../lib/widgets/canvas/office_upgrade_helpers.dart)) |
| **Tier pricing rebalance** (game-designer session: old upgradeCost + 30% Σ(expansion track); small 1000→**4000**, modern 8000→**35000**, techHub 50000→**220000**, campus 500000→**2 700 000** WIP-placeholder; galley 80000 без змін) | `[DONE]` | 2026-07-14 — [lib/models/game_economy.dart](../lib/models/game_economy.dart) `upgradeCost` |
| **Tests** (`office_expansion_test.dart` + `expansion_placement_test.dart` видалено; adapted: game_state / office_game_state / room_taxonomy / shop_panel(_helpers) / office_upgrade_(dialog\|helpers) / build_mode_logic(+cursor) / ghost_invalid_reason_label / corridor_pathfinding / pixel_office_painter_vignette / game_persistence / office_simulation_service; +v6→v7 migration+refund тести) | `[DONE]` | 2026-07-14 — 2432 зелені |

**Залежності та risk-watch:**
- Phase D (Custom Agent Spawn) не залежить від expansion mechanic. Coupling був строго локальний у canvas-layer + economy model.
- **⚠️ Campus grid перед анлоком:** fixed lot 22×20 (inner 20×18=360 tiles) НЕ вміщує 100 агентів (найщільніша teamFloor-кімната = 2.92 tiles/desk → ≥292 tiles лише на столи, без коридорів+support+luxury zones). Campus досі `isWipComingSoon`, тож не блокує. Перед виходом з WIP: або грід ~26×22 (inner 480), або `maxAgents` 100→~70 (game-designer рекомендує грід, не заниження — 100 агентів = fantasy top-tier). Прайс 2.7M — placeholder.
- **Galley** (parallel option, поза лінійним ланцюгом) теж мав `expansions` — тепер fixed 22×54. Rip зачепив його властивості; grid зафіксовано на старому max.
- 3 pre-existing pathfinding-тести (`office_game_state_pathfinding_test`) червоні і на develop (`TilePos` без `==` override) — не пов'язані зі Stage 4.

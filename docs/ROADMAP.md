...
| Arkanoid візуальний reskin під office-style | `[WIP]` | 2026-05-06 — Stage 1: фон → 16-px dark navy checker + vignette (без сканлайнів/guides); cells stretched з 28×13 на 32×16 (13×8 grid збережено, рівні не переробляли); 8 brick-метафор офісу (desk+monitor row-tinted за skinDefault.clothes, whiteboard з cracked-state, trophy plaque, server-rack з LED, safe з rivets); 1-px cyan-accent rim (`#00C0D1` 22% opacity) як силует-сепаратор замість DX-Ball gloss; 2-frame hit-flash для тактильного feedback-у. Stage 2 (paddle/ball/powerups), Stage 3 (декорації + custom-art icons) — follow-up. [lib/widgets/easter_eggs/arkanoid_game.dart](../lib/widgets/easter_eggs/arkanoid_game.dart) |

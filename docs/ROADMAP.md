...
| **Server respawn fallback** — при boot launcher-server scan-ить `AgentRun` записи зі `status=running` → mark як `interrupted`; UI показує banner "Виконання перервано рестартом сервера, повторити?" (українською) | `[PARTIAL]` | 2026-05-17 — server-side частина зроблена: на boot `agentRunStore.load()` + `markRunningAsInterrupted("server-respawn")` промотує всі orphan-running рядки у `interrupted` ([server/src/server.ts](../server/src/server.ts) одразу після `agentRunStore` instantiation). UI banner-частина залишається TODO — потребує `runs?since=` API + `_lastSeenRunId` cursor у клієнті щоб знати які runs показувати. |

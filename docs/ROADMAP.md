...
| Arkanoid візуальний reskin під office-style | `[WIP]` | 2026-05-06 — Stage 1 (bg + bricks): фон → 16-px dark navy checker + vignette; cells 32×16 (13×8 grid збережено); 8 brick-метафор офісу (desk+monitor row-tinted за skinDefault.clothes, whiteboard, trophy, server-rack, safe); 1-px cyan-rim силует-сепаратор; 2-frame hit-flash. Stage 2a (powerups): pixel-art icon-glyph замість monospace-text у falling-pill; pill bumped 34×13 → 38×18; 8 кольорів узгоджено з skinDefault.clothes; HUD-chips і laser-bullet (cyan `#00C0D1`) синхронізовано з тією самою палітрою. Stage 2b (glyph rescale): `_paintGlyph` рендерить `#` як 2×2 px замість 1×1 (`scale=2.0`); bitmap-сітки 9×4..7 переписано → glyph занимає ~70-80% корисної площі pill-а; читабельно при `_puFallSpeed=1.8 px/frame`. Stage 2c (collision feedback): brick hit-flash alpha 0.05 → 0.12 (тепер видно); 2×2 px debris (4 шт normal, 8 шт blast) з 7-tick gravity-fall + fade у кольорі brick-а; 2-px white paddle rim flash на bounce (5 tick); gold pillars+glow на paddle при sticky catch (6 tick) + gold ring навколо stuck ball; 3-frame fading ball-trail (alpha 0.04→0.10). Stage 3 (stone+crystal bricks) — follow-up. [lib/widgets/easter_eggs/arkanoid_game.dart](../lib/widgets/easter_eggs/arkanoid_game.dart) |

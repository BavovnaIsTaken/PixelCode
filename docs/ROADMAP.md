...
| **Persistent `AgentRun` entity** — `{runId, agentId, status: pending\|running\|completed\|failed\|interrupted\|cancelled, userMessageId, partialOutput, finalOutput, toolCalls[], startedAt, completedAt, usage}`; JSONL append-only store | `[TODO]` | Q3 2026 |
| **Periodic partial-assistant flush у ChatHistory** — на кожен `assistant_message_delta`, не лише на `_done`; гарантує що partial видно при reconnect | `[TODO]` | Q3 2026 |
...
| **Server respawn fallback** — при boot launcher-server scan-ить `AgentRun` записи зі `status=running` → mark як `interrupted`; UI показує banner "Виконання перервано рестартом сервера, повторити?" (українською) | `[TODO]` | Q3 2026 |
| **`runs?since=<lastRunId>` API** — клієнт при reconnect отримує всі runs, що відбулись offline | `[TODO]` | Q3 2026 |
| **UI status pill** — "agent running 12m" поряд з sticky "thinking…" індикатором, з reconnect affordance | `[TODO]` | Q3 2026 |
| **Регресійний тест: kill ws mid-run → reopen client → assistant text присутній** | `[TODO]` | Q3 2026 |
...
| **Multi-client sync tests** — same-message-from-2-devices ≡ 2-messages-from-1-device; idempotent merge через localId | `[TODO]` | Q3 2026 |
| **Board transitions — server as single writer** (demote `task_progress_provider` simulator з drive→reflector; перенести RNG `rollOutcome` на сервер; column transitions тригеряться з run-completion signal — `AgentRunner` finish / `CircuitBreaker` trip — не з client-side timer; client продовжує малювати progress animation як heartbeat-reflector над board state; reuse `subagent_thread_event` pattern landed 2026-05-13); прибирає drift між manager-LLM `board_move_task` і client-side simulator-move; prerequisite для Done UX v2 schema v6 (поле `outcome` мусить бути persisted server-side, інакше client-RNG не serializable без race). Файли під заміну: [lib/providers/task_progress_provider.dart](../lib/providers/task_progress_provider.dart) (drive→reflect), [lib/services/task_outcome.dart](../lib/services/task_outcome.dart) (move до server), `server/src/agent_runner.ts` (emit transition на finish), `server/src/board.ts` (validate + apply server-side) | `[TODO]` | Q3 2026 |
| **Empirical baselines analysis** (заплановано на +30 днів ПІСЛЯ C.2 ship — раніше дані порожні) — обчислити median/p95 per `{role, taskType}` з `usage_log.jsonl`; soft warning у facilitator UI на 2× median outliers; **не LLM self-estimate**, emprical distribution. Trigger: schedule task коли C.2 merged | `[TODO]` | Q3 2026 + 30d |

...
| **Persist canvas pan + zoom** (`TransformationController.value` (Matrix4) зберігати у SharedPreferences після idle 500ms; restore на mount; per-tier key (`office_view_transform_tier{N}`) щоб після `upgradeOffice` не відновлювався offset поза новою сіткою; build-mode-reset поведінка не змінюється — persist лише "regular view" transform; clamp на restore проти changed grid bounds) | `[TODO]` | Q3 2026 — зараз `AgentCanvas` створюється заново на кожен switch табів (Hub mobile tabs / Roster ↔ Office) і `_transformController` стартує з identity → користувач втрачає "де покинув". Файли: [lib/widgets/canvas/agent_canvas.dart](../lib/widgets/canvas/agent_canvas.dart) (новий `_loadCanvasTransform`/`_saveCanvasTransform` поряд з `_loadForemanIntroFlag` — той самий SharedPreferences паттерн); тест — нова `test/widgets/canvas_transform_persistence_test.dart` з roundtrip + clamp-on-tier-shrink fixture. Залежність: не блокує D, але добра передумова для D.1 "canvas як сцена" (named characters мають жити в офісі який гравець вже скомпонував). |
